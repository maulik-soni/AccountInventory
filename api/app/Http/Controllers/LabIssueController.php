...
class LabIssueController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $lab->status = Input::get("status");
		$lab->save();
		return ("you data submitted successfully");
    }
    ...
    public function repoLabissue(Request $request)
    {   
    ...
    {   
        $data = $request->all();
        $lab = new \App\LabIssue;
        $mon=LabIssue::select('status')->where('PCS_ID', '=', $data['PCS_ID'])->update(['status'=>'bad']);
        $ton=LabIssue::select('return_date')->where('PCS_ID', '=', $data['PCS_ID'])->update(['return_date'=>getTimestamp()]);
        return 'Done';
